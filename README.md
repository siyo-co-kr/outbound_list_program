...
- **컬럼명 유연 매칭**: 병원마다 다른 컬럼명을 표준 항목에 자동으로 매핑 (`column_utils.py`)
- **전화번호 자동 정제**: 특수문자 제거, 10자리 → 11자리(010) 자동 보정, 형식 검증, 중복 제거
...
2. 암호가 걸려 있다면 비밀번호 입력
3. [제한 명단 추출 및 저장] 클릭 → 결과를 새 엑셀 파일로 저장

---

## ⚠️ 데이터 요구 양식

### 아웃바운드 리스트 필터 (원본 엑셀 **두 번째 시트** 기준)
...
## 💡 참고 사항
- **중복 제거**: 동일한 연락처가 있을 경우, 데이터 무결성을 위해 중복 항목은 자동 제거됩니다.
- **날짜 오류 처리**: 날짜 형식이 잘못된 데이터는 필터링 과정에서 자동으로 제외될 수 있습니다.
